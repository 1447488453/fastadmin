...
use app\common\model\User as User1;
...
class Validate extends Api
{
    ...
    public function check_email_available()
    {
        ...
        $count = User1::where('email', '=', $email)->where('id', '<>', $id)->count();
        if ($count > 0)
        {
    ...
    {  
        $username = $username;
        $count= User1::where('username', '=', $username)->count();
        if ($count > 0){
            $this->error(__('用户名已经被占用'));
        }else{
            if(!preg_match("/^[a-zA-Z]{1}([a-zA-Z0-9]){7}$/",$username)){  
                $this->error(__('用户名要是8位以字母开头的字母+数字组合'));
        }
     ...
     * @param string $id 排除会员ID
     */
    public function check_mobile_available($mobile){
        $mobile = $mobile;
        $count = User1::where('mobile', '=', $mobile)->count();
        if ($count > 0){
    ...
    {
        $email = $this->request->request('email');
        $count = User1::where('email', '=', $email)->count();
        if (!$count)
        {
            $this->error(__('邮箱不存在'));
        }
